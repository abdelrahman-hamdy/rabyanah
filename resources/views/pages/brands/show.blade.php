<x-layout.app :title="$brand->name . ' - Rabyanah'">
    <!-- Hero Section with Archive Hero Component -->
    <x-ui.archive-hero
        :label="__('Brand')"
        :title="$brand->name"
        :subtitle="$brand->description"
        :breadcrumbs="[['label' => __('Brands'), 'url' => route('brands.index')], ['label' => $brand->name]]"
    >
        @if($brand->image_url)
        <div class="flex justify-center mt-10">
            <div class="rounded-2xl overflow-hidden bg-white shadow-xl shadow-gray-200/50 p-6">
    ...
    <section class="pb-20 md:pb-28" style="background-color: #ffffff; padding-top: 80px;">
        <div class="container mx-auto px-4 lg:px-8">
            ...
            <div class="bg-gray-50 rounded-2xl border border-gray-100 p-5 md:p-6 mb-10">
                <form method="GET" action="{{ route('brands.show', $brand) }}" class="flex flex-col lg:flex-row items-stretch gap-4">
                    <!-- Search -->
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 lg:gap-8">
                ...
                <x-ui.product-card :product="$product" :showBrand="false" />
                @endforeach
            </div>
